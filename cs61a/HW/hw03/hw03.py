...
#    direction = 1
#    while index <= n:
#        if num_eights >= 1 or score % 8 == 0:
#            direction = -1
#        score = score + direction
# From top to down to think
...
#    else:
#        maxcoin = 
#        with_part = count_coins(change - maxcoin)
#        maxcoin = next_smaller_coin(maxcoin)
#        without_part = count_coins(change)
# 这里极度想使用函数，但是就是没想到使用higher-order function
    def f(n, m):
        if n < 0:
            return 0
        elif n == 0:
            ...
            return 1
        #不使用m < 0，因为依照next_smaller_coin m=1触底
        else:
            with_m = f(n-m, m)
            without_m = f(n, next_smaller_coin(m))
