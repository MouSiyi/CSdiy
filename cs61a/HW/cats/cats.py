...
def pick(paragraphs, select, k):
    ...
    chosen = [p for p in paragraphs if select(p)]
    if k < len(chosen):
        return chosen[k]
    else:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def relevant(paragragh):
        paragragh = remove_punctuation(paragragh.lower()).split()
        relevant_topics = [t for t in topic if t in paragragh]
        if relevant_topics:
            return True
        else:
            return False
...
def accuracy(typed, source):
    ...
    correct = 0
    if typed_length == source_length == 0:
       acc = 100.0
    elif typed_length == 0 and source_length != 0:
        acc = 0.0
    elif 0 < typed_length <= source_length:
        for i in range(typed_length):
            if typed_words[i] == source_words[i]:
                correct += 1
        acc = correct / typed_length * 100
    else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        if min(diff_list) <= limit:
            return_word = word_list[diff_list.index(min(diff_list))]
        else:
            return_word = typed_word
    return return_word
...
def feline_fixes(typed, source, limit):
    ...
    #Method 2
"""   def substitutes(start, goal, count):
        if count > limit:
            return limit + 1
        #停止递归上界
        if start == '' and goal == '':
        ...
        #停止递归base case
            ...
            return substitutes(start[1:], goal[1:], count + 1)
        elif start[0] != goal[0]:
            return substitutes(start[1:], goal[1:], count + 1)
        else:
            return substitutes(start[1:], goal[1:], count)
    return substitutes(typed, source, 0)"""
    #再一次领略了higher-order function
    ...
    for w in word_indices:
        for i in player_indices:
            if i == 0:
                ...
                continue
            if time(match, i, w) < min_time:
                fastest_one = i
                min_time = time(match, i, w)
        fastest[fastest_one].append(get_word(match,w))
